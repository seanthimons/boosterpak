...
write_default_air_config <- function(path) {
  writeLines(default_air_config_lines(), path, useBytes = TRUE)
  invisible(path)
}

#' Build the default Air formatter configuration
#'
#' @param marked Whether to include the boosterpak ownership marker.
#' @return The canonical Air configuration lines.
#' @noRd
default_air_config_lines <- function(marked = TRUE) {
  lines <- c(
    "[format]",
    "line-width = 120",
    "indent-width = 2",
  ...
  if (isTRUE(marked)) c(air_config_marker(), lines) else lines
}

#' Build the Air ownership marker
#'
#' @return The exact ownership marker line.
#' @noRd
air_config_marker <- function() {
  "# Generated by boosterpak; deinit() removes this file only while unchanged."
}

#' Recognize unchanged boosterpak-generated Air content
#'
#' @param lines Air configuration lines.
#' @return A logical scalar.
#' @noRd
is_generated_air_config <- function(lines) {
  candidates <- list(
    default_air_config_lines(marked = TRUE),
    default_air_config_lines(marked = FALSE),
    c("# air formatter configuration", "")
  )
  any(vapply(candidates, identical, logical(1), y = lines))
}
