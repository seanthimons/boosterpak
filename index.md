...
when possible, installs anything still missing with `pak`, and uses
`renv` for project-local libraries and lockfiles.
...
`air.toml`, manages the `.Rprofile` helper-source line, and can
...
before downloading, then install any remaining packages with `pak`.
Package-only packs stay flat at `boosters/packs/<name>.toml`; packs that
carry copied helper files use `boosters/packs/<name>/<name>.toml` plus
...
installing the remaining declared packages with `pak`.
...
missing direct packages, function drift or missing materialized files,
pack catalog counts, `renv` state, lockfile presence, and the
`.Rprofile` hook.
