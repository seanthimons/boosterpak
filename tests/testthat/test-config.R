test_that("init writes v0.1 config and Rprofile hook when explicit", {
  ...
  expect_equal(
    tail(readLines(file.path(root, ".Rprofile"), warn = FALSE), 3L),
...
test_that("init upgrades historical Rprofile hooks to one managed block", {
  for (historical_hook in c(
    boosterpak:::legacy_rprofile_line(),
    boosterpak:::rprofile_line()
  )) {
    root <- withr::local_tempdir()
    writeLines(
      c("before <- TRUE", historical_hook, historical_hook, "after <- TRUE"),
      file.path(root, ".Rprofile")
    )
    ...
    expect_false(boosterpak:::rprofile_line() %in% lines)
    expect_equal(sum(lines == boosterpak:::rprofile_startup_begin_marker()), 1L)
    expect_equal(sum(lines == boosterpak:::rprofile_startup_end_marker()), 1L)
    expect_equal(tail(lines, 3L), boosterpak:::rprofile_startup_block())
  }
})

test_that("init leaves existing managed Rprofile hook unchanged", {
  root <- withr::local_tempdir()
  withr::local_options(list(
  ...
  writeLines(
    c("before <- TRUE", boosterpak:::rprofile_startup_block(), "after <- TRUE"),
    file.path(root, ".Rprofile")
  )
...
test_that("managed Rprofile hook errors do not abort later profile code", {
  root <- withr::local_tempdir()
  dir.create(file.path(root, "boosters"))
  writeLines(
    'stop("broken attach")',
    file.path(root, "boosters", "attach.R")
  )
  writeLines(
    c(boosterpak:::rprofile_startup_block(), "after <- TRUE"),
    file.path(root, ".Rprofile")
  )
  ...
  expect_error(
    capture.output(sys.source(".Rprofile", envir = env), type = "message"),
    NA
  )
  expect_true(env$after)
})
